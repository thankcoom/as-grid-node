...
class AuthClient:
    ...
    async def register(self) -> Optional[dict]:
        ...
        """
        if not self.auth_server_url or not self.user_id:
            logger.warning("AUTH_SERVER_URL or USER_ID not configured, running in standalone mode")
            return None
